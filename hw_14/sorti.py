...
def sorti_quick(nums):
    ...
    sorti_quick - функция, которая получает индекс
    опорного элемента (pivot)   и делит список относительного него используя
    рокурсивную функцию _sorti_quick.
    """
...
def sorti_insert(nums):
    ...
    sorti_insert - функция проверяет первых два значения в последовательности
    меняя их местами если второе значение меньше первого, далее проверяет
    третье значение списка с первыми двумя размещая его по возрастанию и т.д.
    """
...
def sorti_bubble(nums):
    ...
    sorti_bubble - функция последовательно сравнивает значения соседних
    элементов в списке и меняет их местами, если предыдущее оказывается больше
    последующего. Таким образом элементы с большим значением оказываются
    в конце списка, а с меньшим в начале.
    """
    n = len(nums)
    for i in range(n-1):
